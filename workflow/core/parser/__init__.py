...
Four parsers, one per artifact type. All structured artifacts are JSON
(catalog, roles, trust grants); only the lifecycle uses its own DSL
(stateDiagram-v2 mermaid).

- `mermaid` — lifecycle `.mermaid` files into `Lifecycle`.
