"""Lifecycle model — states, transitions, and the diagram-level metadata.

Mirrors the framework's `state-machine-principles.md`. The mermaid parser produces
a `Lifecycle` from a `.mermaid` file; the validator and operations consume it.
"""
...
class TransitionType(Enum):
    ...
    CROSS_PROCESS = "cross_process"  # to/from another process's lifecycle


@dataclass(frozen=True)
class State:
    """A node on the lifecycle diagram.
    ...
    states). It is declared structurally in the lifecycle file via a note
...
class Transition:
    ...
    lifecycle file. The HCP catalog row corresponding to this transition lives
...
@dataclass
class Lifecycle:
    """A parsed lifecycle .mermaid file.
    ...
    def state(self, name: str) -> State:
        if name not in self.states:
            raise KeyError(f"State {name!r} not found in lifecycle {self.name!r}")
