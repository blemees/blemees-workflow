...
The original model satisfied both with **two labels** per relationship: a dependent-side back-pointer (`parent-of:<parent>` / `collected-by:<collector>`) for direction 1, and an anchor-side registry (`subprocess:<child>` / `collects:<contributor>`) for direction 2. The anchor thus carried its cohort, readable in a single `read_issue(anchor)`.
...
Record each relationship with a **single dependent-side label** — `parent-of:<parent>` on each child, `collected-by:<collector>` on each contributor. Remove the anchor-side registries (`subprocess:`, `collects:`).

- The **cohort** is discovered on demand by `list_issues(label="parent-of:<parent>")` / `list_issues(label="collected-by:<collector>")`, not read off the anchor.
...
This decision also reshapes the operations that create these relationships (candidate #1): with no anchor-side write, **`spawn-issue` collapses to a single-issue create** (make the child, stamp `parent-of:`), and **`collect-into` collapses to a single existing-issue mutation** (stamp the contributor `collected-by:`). The anchor is touched only later, by the cascade.
