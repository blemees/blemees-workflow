...
**generated** visualization regenerated by `workflow render-mermaid`.
...
`workflow render-mermaid [<name>]` regenerates `<name>-states.mermaid`
from the JSON. The output is deterministic — two runs produce byte-identical
text — so the checked-in mermaid stays in sync via a pre-commit hook or CI
check.
...
Authors never edit the mermaid file. The pre-commit / CI flow is:

```bash
$ # edit refinement-states.json
$ workflow validate
$ workflow render-mermaid refinement
$ git add refinement-states.json refinement-states.mermaid
```
