...
- [`postmortem`](./postmortem.md) — Document the timeline, root cause, and remediation for an incident. Spawned by incident-response on stabilization; closes at `complete`. On close, the PM files bug/feature follow-ups on refinement and chore follow-ups on inner-loop via the closing-state spawn declaration.
