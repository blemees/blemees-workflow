...
def test_view_issue_json_includes_next_actions(
    workflow_dir: Path,
    capsys: pytest.CaptureFixture,
) -> None:
    """`view-issue --json` serializes next actions. Regression: the projection
    read `a.triggering_role` (singular) and crashed with AttributeError on any
    issue whose state has outgoing transitions."""
    with (
        mock.patch(
            "workflow.backends.github.GitHubBackend.read_issue",
            return_value=_fake_state(state_name="raw"),
    ...
    assert rc == 0, output
    payload = json.loads(output)
    assert payload["next_actions"], "raw has outgoing transitions"
    assert "triggering_roles" in payload["next_actions"][0]
    assert isinstance(payload["next_actions"][0]["triggering_roles"], list)


def test_view_issue_degrades_when_workflow_unresolvable(
    tmp_path: Path,
    capsys: pytest.CaptureFixture,
) -> None:
    """A claimed issue whose state isn't in any known workflow still renders.
    Regression: `context` was unbound on the WorkflowError path and the
    next-actions block dereferenced it (UnboundLocalError)."""
    from workflow.backends.base import IssueState

    state = IssueState(
        issue_id="123",
        state="mystery_state",
        agent_claim="developer",
        last_state="raw",
    )
    with (
        mock.patch(
    ...
    output = capsys.readouterr().out
    assert rc == 0, output
    assert "mystery_state" in output


def test_claim_resolves_agent_role_from_flag(
    workflow_dir: Path,
    capsys: pytest.CaptureFixture,
) -> None:
    """The global `--agent-role` flag satisfies claim-issue. Regression: the
    handler consulted only the agent home's config.json, so the documented
    flag > env > config cascade failed for claim/advance."""
    with mock.patch(
        "workflow.backends.github.GitHubBackend.read_issue",
        return_value=_fake_state(state_name="raw"),
    ):
...
def test_advance_role_mismatch_from_flag_rejected(
    workflow_dir: Path,
    capsys: pytest.CaptureFixture,
) -> None:
    """`--agent-role` reaches the planner's role validation on advance over a
    claim transition. Regression: the actor resolved to None (config.json
    only), and a None actor skips role validation entirely."""
    with mock.patch(
        "workflow.backends.github.GitHubBackend.read_issue",
        return_value=_fake_state(state_name="raw"),
    ):
